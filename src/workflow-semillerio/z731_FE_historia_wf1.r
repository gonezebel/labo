...
PARAM  <- list()
PARAM$experimento <- "FE7310_wf1"

PARAM$exp_input  <- "DR7210_ninguno_3var"
...
PARAM$RandomForest$semilla  <- 874793    # cambiar por la propia semilla
...
PARAM$CanaritosAsesinos$semilla  <- 874793   # cambiar por la propia semilla
...
CanaritosAsesinos  <- function( canaritos_ratio=0.2, canaritos_desvios=3.0, canaritos_semilla=999983 )
{
  ...
  umbral  <- tb_importancia[ Feature %like% "canarito", 
                             median(pos) +  7*sd(pos) ]  #Atencion corto en la mediana mas desvios!!
